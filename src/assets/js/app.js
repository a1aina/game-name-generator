...
function getInfluences(config) {
  ...
  for (const value of raw) {
    const family = influenceMap[value];
...
function softenJoins(value) {
  ...
  name = name.replace(
    /([bcdfghjklmnpqrstvwxyz]{4,})/g,
    (match) => `${match.slice(0, 2)}a${match.slice(2)}`
  );
...
function scoreName(name, config) {
  ...
  if (/^(aa|ee|ii|oo|uu)/.test(lower)) {
    score -= 12;
  }

  if (/(aeae|arar|oror|unun|isis)/.test(lower)) {
    score -= 18;
  }
...
function mutateFragment(fragment) {
  ...
  if (vowelIndex >= 0 && chance(0.7)) {
    chars[vowelIndex] = vowelSwaps[chars[vowelIndex]];
  }
...
function generateFantasy(config, mode = "generated", reference = null) {
  ...
  for (let attempt = 0; attempt < 45; attempt += 1) {
    ...
    if (mode === "similar" && reference?.meta) {
      ...
      if (keepStart) {
        start = chance(0.72)
          ...
          : mutateFragment(reference.meta.start);

        anchor = "opening sound";
      } else {
        end = chance(0.72)
          ? reference.meta.end
          : mutateFragment(reference.meta.end);

        anchor = "ending sound";
      }

      if (chance(0.3)) {
        middle = reference.meta.middle;
      }
    }

    if (mode === "direction") {
      const alternatives = profile.patterns.filter(
        (item) => item !== reference?.meta?.pattern
      );

      pattern = randomItem(
        alternatives.length ? alternatives : profile.patterns
      );
      ...
      if (reference?.meta) {
        ...
        const availableStarts = pools.starts.filter(
          (item) => item !== avoidStart
        );

        const availableEnds = pools.ends.filter(
          (item) => item !== avoidEnd
        );

        start = randomItem(
          availableStarts.length ? availableStarts : pools.starts
        );

        end = randomItem(
          availableEnds.length ? availableEnds : pools.ends
        );
      }
    }

    let raw = applyPattern(pattern, {
      start,
      middle,
      end
    });
    ...
    if (
      config.inspiration &&
      chance(mode === "similar" ? 0.35 : 0.55)
    ) {
      const inspired = pools.inspired;

      if (inspired && !raw.startsWith(inspired)) {
        raw = chance(0.6)
          ? `${inspired}${raw.slice(Math.min(2, raw.length))}`
          : raw;
      }
    }
...
function buildWhy(config, result, mode) {
  if (isEmptyConfig(config)) {
    return resultMessages.ordinary;
  }
  ...
  if (result.meta.families?.length) {
    ...
    const descriptions = result.meta.families
      .slice(0, 2)
    ...
    if (descriptions.length) {
      parts.push(
        `leans toward ${descriptions.join(" and ")}`
      );
    }
  }
  ...
  if (config.nameStyle) {
    parts.push(
      `uses the selected ${config.nameStyle} name style as a light influence`
    );
  }

  if (config.inspiration) {
    parts.push(
      `and carries a small sound echo derived from “${config.inspiration}” rather than inserting it literally`
    );
  }

  if (mode === "similar" && result.meta.anchor) {
    parts.push(
      `while preserving the previous ${result.meta.anchor}`
    );
  }

  if (mode === "direction") {
    parts.push(
      "while deliberately changing the prior construction pattern"
    );
  }
...
function generate(config, mode = "generated", reference = null) {
  if (isEmptyConfig(config)) {
    const available = ordinaryNames.filter(
      (name) => !usedNames.has(name.toLowerCase())
    );

    const name = randomItem(
      available.length ? available : ordinaryNames
    );
    ...
    return {
      ...
      meta: {
        profile: "ordinary",
        pattern: "ordinary",
        start: name.slice(0, 1).toLowerCase(),
        middle: "",
        end: name.slice(-2).toLowerCase()
      }
    };
  }

  const generated = generateFantasy(
    config,
    mode,
    reference
  );
...
function renderResult(result) {
  ...
  resultNote.textContent =
    resultMessages[result.mode] ||
    resultMessages.generated;

  whyText.textContent = buildWhy(
    result.config,
    result,
    result.mode
  );
  ...
  requestAnimationFrame(() => {
    resultStage.classList.add("generated");
  });

  if (window.matchMedia("(max-width: 760px)").matches) {
    resultStage.scrollIntoView({
      behavior: "smooth",
      block: "center"
    });
  }
...
function loadShortlist() {
  try {
    const parsed = JSON.parse(
      sessionStorage.getItem("character-name-shortlist") || "[]"
    );
...
function saveCurrentName() {
  ...
  const alreadySaved = shortlist.some(
    (item) =>
      item.name.toLowerCase() ===
      currentResult.name.toLowerCase()
  );
...
function renderShortlist() {
  ...
  for (const item of shortlist) {
    ...
    li.className =
      `shortlist-item${item.final ? " final" : ""}`;
    ...
    for (const [action, label] of actionData) {
      ...
      button.type = "button";
      button.className =
        `shortlist-action${action === "remove" ? " remove" : ""}`;
...
form.addEventListener("reset", () => {
  window.setTimeout(() => {
    ...
    resultNote.textContent =
      "Choose as much or as little as you like, then generate a name.";
...
shortlistElement.addEventListener("click", (event) => {
  ...
  if (button.dataset.action === "final") {
    ...
    shortlist = shortlist.map((entry) => ({
      ...entry,
      final: willSelect && entry.id === item.id
    }));
